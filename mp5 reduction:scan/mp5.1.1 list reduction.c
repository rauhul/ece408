...
__global__ void total(float *input, float *output, int len) {
  ...
  __shared__ float reductionSegment[BLOCK_SIZE];

  int inputIndex = tx + (bx*BLOCK_SIZE);// + (k*BLOCK_SIZE);
  float inputValue = inputIndex < len ? input[inputIndex] : 0;
  reductionSegment[tx] = inputValue;
  __syncthreads();
  ...
  for (int stride = 1; stride < BLOCK_SIZE; stride *= 2) {
    ...
    if (tx % (stride*2) == 0) {
      reductionSegment[tx] += reductionSegment[tx + stride];
    }
    __syncthreads();
  }
